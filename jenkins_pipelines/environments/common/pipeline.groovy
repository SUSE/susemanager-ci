        ...
        try {
            withCreds {
                stage('Clone terracumber, susemanager-ci and sumaform') {
                    ...
                    // run minima sync on mirror (CI acceptance jobs only)
                ...
                stage('Deploy') {
                    if (params.run_deployment) {
                        ...
                        if (params.terraform_taint) {
                            switch(params.sumaform_backend) {
                                ...
                                case "aws":
                                    ...
                                    break
                                default:
                                    error("ERROR: Unknown backend ${params.sumaform_backend}")
                                    break
                            }
                        }
                        ...
                        """
                        deployed = true
                        // Collect and tag Flaky tests from the GitHub Board (with CI label on acceptance jobs)
