def run(params) {
    timestamps {
        ...
        stage("Upload ssh key to local mirror") {
            ...
            sh "ssh -o StrictHostKeyChecking=no ${remote.user}@${mirror_hostname_local} 'scp -o StrictHostKeyChecking=no -r -i /root/testing-suma.pem /srv/mirror ec2-user@${mirror_hostname_aws}:/srv/mirror' "
        }
    }
}
