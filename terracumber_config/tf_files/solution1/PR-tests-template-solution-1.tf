...
provider "libvirt" {
  uri = "qemu+tcp://${var.hypervisor}/system"
}
...
module "cucumber_testsuite" {
  ...
  use_avahi    = false
  name_prefix  = "suma-pr${var.environment_number}-"
  ...
  host_settings = {
    ...
    kvm-host = {
      ...
      additional_grains = {
        hvm_disk_image = {
          leap = {
            hostname = "suma-pr${var.environment_number}-min-nested"
  ...
  nested_vm_host = "suma-pr${var.environment_number}-min-nested"
  nested_vm_mac =  var.mac["nested-vm"]
  provider_settings = {
    pool               = "ssd"
    network_name       = null
    bridge             = "br1"
