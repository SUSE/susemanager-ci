...
variable "MAIL_SUBJECT" {
  type = string
  default = "Results Salt Shaker - products:next - SLES15SP4 Salt Bundle $status: $tests scenarios ($failures failed, $errors errors, $skipped skipped, $passed passed)"
}
...
variable "MAIL_SUBJECT_ENV_FAIL" {
  type = string
  default = "Results Salt Shaker - products:next - SLES15SP4 Salt Bundle: Environment setup failed"
}
