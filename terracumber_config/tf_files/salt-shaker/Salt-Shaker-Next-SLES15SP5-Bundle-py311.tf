...
variable "MAIL_SUBJECT" {
  type = string
  default = "Results Salt Shaker - products:next:python311 - SLES15SP5 Salt Bundle $status: $tests scenarios ($failures failed, $errors errors, $skipped skipped, $passed passed)"
}
...
variable "MAIL_SUBJECT_ENV_FAIL" {
  type = string
  default = "Results Salt Shaker - products:next:python311 - SLES15SP5 Salt Bundle: Environment setup failed"
}
...
module "salt-shaker-products-next" {
  ...
  image              = "sles15sp5o"
  salt_obs_flavor    = "bundle:next:python311"
}
