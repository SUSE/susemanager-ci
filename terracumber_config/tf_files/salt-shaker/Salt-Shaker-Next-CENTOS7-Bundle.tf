...
variable "URL_PREFIX" {
  type = string
  default = "https://ci.suse.de/user/manager/my-views/view/Salt%20Shaker/job/manager-salt-shaker-products-next-centos7-bundle"
}
...
variable "MAIL_SUBJECT" {
  type = string
  default = "Results Salt Shaker - saltstack:products:next - CentOS7 Salt Bundle $status: $tests scenarios ($failures failed, $errors errors, $skipped skipped, $passed passed)"
}
...
variable "MAIL_SUBJECT_ENV_FAIL" {
  type = string
  default = "Results Salt Shaker - saltstack:products:next - CentOS7 Salt Bundle: Environment setup failed"
}
