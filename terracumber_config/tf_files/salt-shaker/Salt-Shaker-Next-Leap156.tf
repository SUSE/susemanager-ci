...
variable "URL_PREFIX" {
  type = string
  default = "https://ci.suse.de/user/manager/my-views/view/Salt%20Shaker/job/manager-salt-shaker-products-next-leap156"
}
...
variable "MAIL_SUBJECT" {
  type = string
  default = "Results Salt Shaker - saltstack:products:next - Leap 15.6 $status: $tests scenarios ($failures failed, $errors errors, $skipped skipped, $passed passed)"
}
...
variable "MAIL_SUBJECT_ENV_FAIL" {
  type = string
  default = "Results Salt Shaker - saltstack:products:next - Leap 15.6: Environment setup failed"
}
...
module "salt-shaker-products-next" {
  ...
  name               = "salt-shaker-products-next-leap156"
  image              = "opensuse156o"
  salt_obs_flavor    = "saltstack:products:next"
  provider_settings  = {
    mac = "aa:b2:93:02:02:0c"
  }
}
